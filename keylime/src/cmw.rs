...
pub fn build_event_log(ima_list_str: &str, mb_list_b64: Option<&str>) -> Vec<u8> {
    ...
    for line in ima_list_str.lines() {
        ...
        let pcr_index_u8: u8 = pcr_index_str.parse().unwrap_or(10); // <--- Parse as u8
        ...
        if let Some((hash_alg, hash_val)) = template_hash.split_once(':') {
            if let Ok(digest_bytes) = hex::decode(hash_val) {
                ...
                // IndexType (type=1, value=PCR index)
                let index_tlv = encode_tlv(TYPE_INDEX_TYPE, &[pcr_index_u8]); // <--- Write as single u8 byte

                // Digest Type: 0x0b for sha256 (simplified)
                ...
                let data_str = format!("{} {}", template_hash, path);
                let data_tlv = encode_tlv(TYPE_TEMPLATE_DATA, data_str.as_bytes());
                let content_ima = encode_tlv(TYPE_CONTENT_IMA, &[name_tlv, data_tlv].concat());
    ...
    if let Some(mb64) = mb_list_b64 {
        if let Ok(decoded) = general_purpose::STANDARD.decode(mb64) {
            ...
            // For the second record (MB), also use a single byte for PCR index 0
